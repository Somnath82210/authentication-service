...
export async function register(
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "catch-error in controller" });
  }
}
export async function userAdminController(
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    let services: any = await adminPriviledgeService(token, data);
    if (services.status === true) {
      res
        .status(200)
        .json({
...
export async function getUserSettingsController(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export async function updateUserController(
  ...
  try {
    ...
    if (services.status === true) {
      res
        .status(200)
        .json({ success: true, message: services.message, id: services.id });
    } else if (services.status === false) {
      res.status(400).json({ success: false, error: services.message });
    }
  } catch (error) {
