  ...
  try {
    ...
    if (services.status === true) {
      res.status(200).json({
        ...
        token: services.data.token,
        email: services.data.email,
        statusCode: services.data.statusCode,
      });
    ...
    res
      .status(500)
      .json({ success: false, error: "email or password missmatch" });
  }
}

export async function adminLevelCheckController(
  req: Request,
  res: Response,
  next: NextFunction
) {
